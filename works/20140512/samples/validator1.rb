...
class Validator
  ...
  def initialize type
    ...
    @rules = {
      mandatory: Proc.new{|key, val, skey, sval| false},
      min_length: Proc.new{|key, val, skey, sval| "The length of #{key} should be more than #{sval}." if val.length < sval},
      email: Proc.new{|key, val, skey, sval| "Invalid format '#{val}' as email." unless val =~ /^[0-9a-zA-Z\._]+@[0-9a-zA-Z\._]+$/},
    }
  end
